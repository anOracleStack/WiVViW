...
import type { Candidate, ChapterReport, Quote, TrendItem, Vitals } from "./types";
...
export const useEngineStore = create<State>((set, get) => ({
  ...
  actions: {
    ...
    setCandidateStatus: (id, status, note) => {
      const candidates = get().candidates.map((c) => c.id === id ? { ...c, status, notes: note ? [note, ...c.notes].slice(0, 6) : c.notes } : c);
