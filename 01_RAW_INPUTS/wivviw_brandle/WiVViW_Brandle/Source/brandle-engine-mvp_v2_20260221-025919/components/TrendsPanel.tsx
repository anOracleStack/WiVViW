          ...
          <div className="mt-2 space-y-3">
            {trends.length ? trends.map((t) => (
              <div key={t.id} className="rounded-xl border border-black/10 bg-white/70 p-3">
                ...
                <div className="mt-2 text-[11px] text-black/40 flex items-center justify-between">
                  <span>Updated: {t.updatedAt}</span>
                  {t.source ? <a className="underline" href={t.source} target="_blank" rel="noreferrer">Source</a> : null}
                </div>
              </div>
            )) : <div className="text-sm text-black/50">No trends loaded.</div>}
          </div>
        </div>
      ) : null}
    </div>
